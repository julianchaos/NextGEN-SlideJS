...
/**
 * Plugin Name: NextGEN Gallery +SlideJS
 * Description: Edição do NextGEN Gallery usando o SlideJS como portfolio
 * Version: 1.0
 * Author: Julian Andrade
 * Plugin URI: http://www.ndrade.com.br
 * Author URI: http://www.ndrade.com.br
 * License: GPLv2
 */
...
//			}
	...
	function _load_portfolio()
	{
		add_shortcode("ngg_portfolio", array(&$this, 'ngg_portfolio'));
		
		wp_enqueue_script("jquery");
		
		wp_register_script('ngg_portfolio_core', plugins_url('portfolio/jquery.slidejs.min.js', __FILE__), array("jquery"));
		wp_enqueue_script('ngg_portfolio_core');

		wp_register_script('ngg_portfolio_init', plugins_url('portfolio/slidejs.initialize.js', __FILE__));
		wp_enqueue_script('ngg_portfolio_init');
		
		wp_register_style('ngg_portfolio_example', plugins_url('portfolio/css/slidejs.css', __FILE__));
		wp_enqueue_style('ngg_portfolio_example');
	}
	function ngg_portfolio()
	{
		global $nggdb;
		
		$fullList = array();
		$albumlist = $nggdb->find_all_album();
		foreach($albumlist as $album)
		{
			$fullList[$album->id] = array("nome"=>$album->name, "galleries"=>array());
			
			foreach($album->galleries as $gallery_id)
			{
				$gallery = $nggdb->find_gallery($gallery_id);
//				var_dump($gallery);
				$fullList[$album->id]["galleries"][$gallery_id] = $gallery->title;
			}
		}
		
		return $this->build_ngg_portfolio($fullList);
	}	
	private function build_ngg_portfolio($albumList)
	{
		global $nggdb;
		
		$selectedGallery = $_GET['gallery-id'];
		$galleryListElement	 = '';
		foreach($albumList as $album)
		{
			$galleryListElement .= "<li class='album'>{$album['nome']}</li>";
			
			foreach($album['galleries'] as $galleryId=>$galleryName)
			{
				if(is_null($selectedGallery)) $selectedGallery = $galleryId;
				
				$selectedCssClass = ($selectedGallery == $galleryId ? 'selected' : '');
				$galleryListElement .= "<li class='gallery $selectedCssClass'>";
					$galleryListElement .= "<a href='?gallery-id=$galleryId'>$galleryName</a>";
				$galleryListElement .= "</li>";	
			}
		}
		
		$pictures = $nggdb->find_images_in_gallery($selectedGallery);
		$imageElements = '';
		$imageCount = count($pictures);
		foreach($pictures as $pic)
		{
			$imageElements .= "<img src='{$pic->_ngiw->cached_singlepic_file()}' />";
		}
		
		$html = <<<EOD
			<div class="container-ngg-portfolio">
				<nav>
					<ul>
						$galleryListElement
					</ul>
				...
				<div class='container'>
					<div id='slides'>
						$imageElements
						<a href="#" class="slidesjs-previous slidesjs-navigation"><i class="icon-chevron-left icon-large">teste</i></a>
						<div id="slidesjs-log" class="slidesjs-navigation"><span class="slidesjs-slide-number"></span>$imageCount</div>
						<a href="#" class="slidesjs-next slidesjs-navigation"><i class="icon-chevron-right icon-large">teste</i></a>
					</div>
				</div>
			</div>
EOD;
